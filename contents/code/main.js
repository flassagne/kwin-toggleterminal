...
function loadConfiguration() {
  ...
  config.launchCommand = readConfig(
    "launchCommand",
    "/usr/bin/foot",
  ).toString();
...
function isTerminal(window) {
  return (
    window.caption.substr(0, config.windowNamePrefix.length) ===
      config.windowNamePrefix &&
    window.caption.substr(
      -1 * config.windowNameSuffix.length,
      config.windowNameSuffix.length,
    ) === config.windowNameSuffix &&
